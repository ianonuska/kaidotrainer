...
func draw_tree_large(x: float, y: float):
	if tex_grass_biome:
		# Use grass biome sprite (contains trees at specific positions)
		var src = Rect2(0, 0, 48, 64)  # Adjust based on actual sprite layout
		canvas.draw_texture_rect_region(tex_grass_biome, Rect2(x, y, 48, 64), src)
		return
	# Simple colored tree
