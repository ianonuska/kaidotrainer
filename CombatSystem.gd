...
func deal_damage_to_tunnel_robot(idx: int, damage: int):
	...
	var r = game.tunnel_robots[idx]
	if r.defeated:
		return
	
	r.hp -= damage
	game.add_hit_effect(r.pos + Vector2(0, -30), str(damage), Color(1.0, 0.9, 0.3))
	...
	if r.hp <= 0:
		r.hp = 0
		r.defeated = true
		game.add_hit_effect(r.pos + Vector2(0, -40), "DEFEATED!", Color(0.3, 1.0, 0.5))
...
func end_tunnel_fight_victory():
	...
	game.tunnel_fight_active = false
	...
	game.dialogue_queue = [
		...
		{"speaker": "quest", "text": "Explore the hideout"},
	]
	game.next_dialogue()
...
func start_player_heavy_attack():
	...
	game.combat_player_state_timer = 0.5
...
func draw_combat_arena():
	...
	# Controls
	game.draw_string(ThemeDB.fallback_font, Vector2(10, 315), "[X] Strike  [_] Heavy  [O] Evade  △ Counter  R2 Gadget", HORIZONTAL_ALIGNMENT_LEFT, -1, 10, Color(0.5, 0.5, 0.5))
			...
			"dodging":
				...
				for i in range(3):
					...
					if walk_frames.size() > 0:
						...
						var ghost_dest = Rect2(ghost_x - gw/2, pos.y - gh + 10, gw, gh)
...
func draw_combat_robot(pos: Vector2):
	...
	var frame = int(game.continuous_timer * 6) % 4
	var row = 0
	var frame_size = 16
	var scale_factor = 3.0
	
	var tex_width = game.tex_robot_enemy.get_width()
	if tex_width >= 64:
		frame_size = tex_width / 4
	else:
		frame_size = tex_width
		frame = 0
	
	var flip = pos.x > game.combat_player_pos.x
	...
	match game.robot_state:
		...
		"telegraph":
			row = 0
			var flash = sin(game.continuous_timer * 15) * 0.5 + 0.5
			var glow_size = 20
			var glow_alpha = 0.4
			match game.robot_current_attack:
				"quick_jab": glow_size = 15; glow_alpha = 0.3
				"baton_swing": glow_size = 30; glow_alpha = 0.5
				"lunge_grab": glow_size = 40; glow_alpha = 0.6
				"combo_strike": glow_size = 25; glow_alpha = 0.45
			game.draw_circle(Vector2(pos.x, pos.y - 20), glow_size, Color(1.0, 0.2, 0.2, flash * glow_alpha))
		"attacking":
			row = 0
			frame = 2
		"hit":
	...
	game.draw_texture_rect_region(game.tex_robot_enemy, dest, src, flash_mod)
	
	if is_flashing:
		var burst_size = 25 * (game.hit_flash_timer / 0.15)
		game.draw_circle(pos + Vector2(0, -20), burst_size, Color(1, 1, 0.8, game.hit_flash_timer * 3))
...
func draw_tunnel_robot(robot: Dictionary, shake_offset: Vector2, index: int):
	...
	# Use sprite instead of procedural drawing
	if not game.tex_robot_enemy:
		return
	
	var flash_mod = Color(1, 1, 1, 1)
	# Different tints for each robot
	var tints = [
		Color(1.0, 0.9, 0.9),   # Reddish
		Color(0.9, 0.9, 1.0),   # Bluish
		Color(1.0, 1.0, 0.9)    # Yellowish
	]
	var base_tint = tints[index % 3]
	
	var frame = int(game.continuous_timer * 6 + index) % 4
	var frame_size = 16
	var scale_factor = 2.5  # Slightly smaller than main robot
	...
	match robot.state:
		...
		"telegraph":
			flash_mod = base_tint
			# Counter indicator
			var flash = sin(game.continuous_timer * 20 + index) * 0.4 + 0.6
			var glow_color = Color(1.0, 0.2, 0.2, flash * 0.7)
			game.draw_circle(Vector2(pos.x, pos.y - 30), 28, glow_color)
			var prompt_color = Color(0.3, 0.9, 1.0, flash)
			game.draw_string(ThemeDB.fallback_font, Vector2(pos.x - 8, pos.y - 70), "△", HORIZONTAL_ALIGNMENT_LEFT, -1, 20, prompt_color)
			var ring_size = 25 + sin(game.continuous_timer * 15 + index * 2) * 4
			game.draw_arc(Vector2(pos.x, pos.y - 20), ring_size, 0, TAU, 24, prompt_color, 2.0)
		"attack":
			flash_mod = base_tint
			frame = 2
			# Attack effect
			var arm_dir = 1 if pos.x > game.combat_player_pos.x else -1
			game.draw_rect(Rect2(pos.x - arm_dir * 10, pos.y - 25, 30 * -arm_dir, 8), Color(1.0, 0.25, 0.25))
		"hit":
...
func draw_defeated_robot(pos: Vector2):
	...
	game.draw_texture_rect_region(game.tex_robot_enemy, dest, src, Color(0.5, 0.5, 0.5, 1))
